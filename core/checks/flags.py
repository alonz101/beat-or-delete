...
def classify_coverage(coverage_ratio, cfg):
...
def compute_flags(
    ...
    if is_lossy:
        ...
        elif top_freq < SPECTRAL_MP3_320_CUTOFF_HZ and bitrate >= cfg.BITRATE_320_THRESHOLD:
    ...
    if meta["bit_depth"] == 24 and integrity["lsb_zero_ratio"] > cfg.LSB_ZERO_RATIO_FAKE_24BIT:
    ...
    if total_events > cfg.CLIP_BLOCKING_EVENTS or max_ms > cfg.CLIP_BLOCKING_MS:
    ...
    elif total_events > cfg.CLIP_MARGINAL_EVENTS or max_ms > cfg.CLIP_MARGINAL_MS:
    ...
    if integrity["dynamic_range_db"] > 0:
        if integrity["dynamic_range_db"] < cfg.DYNAMIC_RANGE_BLOCKING_DB:
        ...
        elif integrity["dynamic_range_db"] < cfg.DYNAMIC_RANGE_MARGINAL_DB:
            flags.append("LOW_DYNAMIC_RANGE")

    if integrity["noise_floor_dbfs"] > cfg.NOISE_FLOOR_MARGINAL_DBFS:
    ...
    if abs(integrity["dc_offset_L"]) > cfg.DC_OFFSET_THRESHOLD or abs(integrity["dc_offset_R"]) > cfg.DC_OFFSET_THRESHOLD:
    ...
    if vinyl and vinyl.get("vinyl_rip"):
        ...
        if wf and wf > cfg.VINYL_WOW_FLUTTER_FLAG:
        ...
        if hum:
            if hum_db and hum_db > cfg.VINYL_HUM_STRONG_DB:
                flags.append("HUM")
                verdict_reasons.append(
    ...
    if loudness["true_peak_L_dbfs"] > cfg.TRUE_PEAK_HOT_DBFS or loudness["true_peak_R_dbfs"] > cfg.TRUE_PEAK_HOT_DBFS:
